...
int16_t CC1101::startReceive() {
  ...
  state = SPIsetRegValue(RADIOLIB_CC1101_REG_MCSM1, RADIOLIB_CC1101_RXOFF_RX, 3, 2);
  RADIOLIB_ASSERT(state);
...
*/
  bool run_loop = true;
  while(run_loop) {
    uint8_t pkt_status = SPIreadRegister(RADIOLIB_CC1101_REG_PKTSTATUS);
    if(pkt_status & RADIOLIB_CC1101_GDO0_ACTIVE) {
      //Serial.println("GDO0 high, waiting for it being low...");
      while(run_loop) {
        uint8_t pkt_status = SPIreadRegister(RADIOLIB_CC1101_REG_PKTSTATUS);
        if(pkt_status & RADIOLIB_CC1101_GDO0_ACTIVE) {

        } else {
          //Serial.println("GDO0 low after high - good.");
          run_loop = false;
  ...
  uint8_t pkt_status = SPIreadRegister(RADIOLIB_CC1101_REG_PKTSTATUS);
  //Serial.print("GDO0: "); Serial.print(pkt_status & RADIOLIB_CC1101_GDO0_ACTIVE);
  //Serial.print("  GDO2: "); Serial.print(pkt_status & RADIOLIB_CC1101_GDO2_ACTIVE);
  //Serial.print("  CRC: "); Serial.print(pkt_status & RADIOLIB_CC1101_CRC_OK);
  //Serial.println();
  //return -666;
  bool ready = false;
  //if(pkt_status & RADIOLIB_CC1101_GDO0_ACTIVE) {
   if(pkt_status & RADIOLIB_CC1101_CRC_OK || pkt_status & RADIOLIB_CC1101_CRC_ERROR) { 
    Serial.print("GDO0: "); Serial.print(pkt_status & RADIOLIB_CC1101_GDO0_ACTIVE);
    Serial.print("  GDO2: "); Serial.print(pkt_status & RADIOLIB_CC1101_GDO2_ACTIVE);
    Serial.print("  CRC: "); Serial.print(pkt_status & RADIOLIB_CC1101_CRC_OK);
    Serial.println();
  ...
  uint8_t bytes_to_read = SPIreadRegister(RADIOLIB_CC1101_REG_RXBYTES);
  Serial.println(millis());
  Serial.print("bytes_to_read = "); Serial.println(bytes_to_read);
    #if 1
  uint8_t tmp_data[255] = { 0 };
  Serial.println("tmp_data: ");
  SPIreadRegisterBurst(RADIOLIB_CC1101_REG_FIFO, bytes_to_read, tmp_data);
  for(int i=0; i<bytes_to_read; i++) {
    Serial.print(tmp_data[i]); Serial.print(", ");
    if(i<len && i>0) {
      data[i-1] = tmp_data[i];
    }
  }
...
//
  ...
  Serial.print(SPIgetRegValue(RADIOLIB_CC1101_REG_FIFO));
  Serial.print(SPIgetRegValue(RADIOLIB_CC1101_REG_FIFO));
  #endif
  ...
  uint8_t begin_bytes_to_read = SPIreadRegister(RADIOLIB_CC1101_REG_RXBYTES);
  if(begin_bytes_to_read > (length) ){
    Serial.print("begin_bytes_to_read = "); Serial.println(begin_bytes_to_read);
  } else {
    //return -666;
  }
  // check address filtering
  ...
  for( int i=0; i<bytes_to_read-1; i++) {
    uint8_t next = SPIgetRegValue(RADIOLIB_CC1101_REG_FIFO);
  ...
  uint8_t tmp_data[255] = { 0 };
  Serial.println("tmp_data: ");
  SPIreadRegisterBurst(RADIOLIB_CC1101_REG_FIFO, bytes_to_read-1, tmp_data);
  for(int i=0; i<bytes_to_read-1; i++) {
    Serial.print(tmp_data[i]); Serial.print(", ");
  }

  #endif
  //Serial.println("Flushing RX...");
  //SPIsendCommand(RADIOLIB_CC1101_CMD_FLUSH_RX);
